...
- Helper: `chuanHM` (chuẩn hoá tên hạng mục, bỏ dấu/hoa thường), `labelsOf`, `optsOf`, `isReq`, `VD` = tiền tố dòng ví dụ `[VÍ DỤ]` (bị bỏ qua khi nhập).
...
5. Kiểm tra: `node --check public/app.js`; tạo file mẫu và parse ngược bằng `importParse(...,'vs')`; test `importCommit` với `server/supa` giả (ghi đè `require.cache`).
